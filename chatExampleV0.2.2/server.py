...
from cryptoFunction import cryptex
from sqlLiteUtility import DatabaseUtility
...
class clientObj:
    ...
    def __init__(self, _nickName: str, _client: socket, _databaseName):
        ...
        if result:
            self.getUserFromDatabase(result)
        else:
            print(f"debug from clientObj: current user: {self.nickname}, {self.ipAddress} is not in the database and "
                  f"need to be registered")
            self.registerUserIntoDatabase()

    def getUserFromDatabase(self, result):
        """
    ...
    def getUserById(self, nickName, ipAddress):
        try:
            ...
            self.cursor.execute("SELECT * FROM users WHERE nickName = ? AND ipAddress = ?", (nickName, ipAddress))
    ...
    def signIn(self, _message):
        pass

    def changePassword(self, _message):
        pass
...
class ServerX:
    ...
    def connectDatabase(self):
        self.db = DatabaseUtility(DATABASE_NAME)
        self.db.printColumName("user")
    ...
    def connectServer(self):
        ...
        try:
            ...
            self.server.listen(5) #is backlog
            print(f"server connected: {self.host}:{self.port}")
            print("waiting for incoming connection...\n")
            self.checkForIncomingClient()
    ...
    def checkForIncomingClient(self):
        """
        Il server sta in loop aspettando che un client si connetta
        :return:
        """
        while True:
            try:
    ...
    def handShake(self, client, address):
        ...
        if nickName not in self.clients and nickName not in self.clients.values():
            handShakeMsg = self.stampMessage("handShake", "ACK", nickName)
            self.sendHandShakeMessage(handShakeMsg, client)
            self.connectClient(client, address, nickName)
        else:
            handShakeMsg = self.stampMessage("handShake", "KICK", nickName)
            self.sendHandShakeMessage(handShakeMsg, client)
            print(f"connection refused with: {nickName}, you need to change your nickname")
            client.close()
    ...
    def handleMessageReception(self, client: clientObj):
        ...
        if client in self.kickedClient and client in self.blacklist:
            ...
            self.handleClientMessages(message, client)
    ...
    def connectClient(self, _client, _address, _nickName: str):
        ...
        :return:
        """
        client = clientObj(_nickName, _client, self.db.name)
    ...
    def handleClientMessages(self, message: dict, client: clientObj):
        """
        ...
        elif message["msgType"] in ["quit", "disconnect"]:
